...
export const MovieSchema = z.object({
    ...
    popularity: z.number().int().min(0, "Popularity must be at least 0").max(5, "Popularity must be at most 5"),
    directorId: z.string().min(1, "Director is required"),
    genreId: z.string().min(1, "Genre is required"),
    youtubeTrailerId: z.string().min(1, "YouTube trailer is required"),
    actorIds: z.array(z.string()).min(1, "At least one actor is required"),
    platformIds: z.array(z.string()).min(1, "At least one platform is required"),
    prizeIds: z.array(z.string()).optional(),
});
